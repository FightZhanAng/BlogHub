    ...
    <div class="comment-list" v-if="comments.length">
      <div v-for="comment in rootComments" :key="comment.id" class="comment-item">
        <div class="comment-avatar">
          <el-avatar :size="36" :src="comment.avatar ? 'http://localhost:8080' + comment.avatar : undefined">{{ comment.nickname[0] }}</el-avatar>
        </div>
            ...
            <template v-for="reply in getReplies(comment.id)" :key="reply.id">
              <div class="reply-item">
                <div class="reply-avatar">
                  <el-avatar :size="28" :src="reply.avatar ? 'http://localhost:8080' + reply.avatar : undefined">{{ reply.nickname[0] }}</el-avatar>
                </div>
                <div class="reply-body">
                  ...
                  <div class="sub-replies" v-if="getReplies(reply.id).length">
                    <div v-for="sub in getReplies(reply.id)" :key="sub.id" class="reply-item">
                      <div class="reply-avatar">
                        <el-avatar :size="26" :src="sub.avatar ? 'http://localhost:8080' + sub.avatar : undefined">{{ sub.nickname[0] }}</el-avatar>
                      </div>
                      <div class="reply-body">
                        <div class="reply-header">
